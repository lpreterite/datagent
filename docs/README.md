...
经过datagent发起请求并获得数据，需要经过多个环节，这里称之为**数据流动过程**。

[此处应该补上图]

上图诠释了大部分流程，在钩子(`hook`)环节由于是动态的，可通过配置增加更多的处理环节。关于数据级别的处理（如：数据类型、默认值）建议放到数据模型的字段类型中进行，再在数据对象中添加相应的钩子进行统一处理，如何实现可查看[自定义字段类型](#自定义字段类型)；而数据结构不能满足UI的需求需要改变数据结构的时候，这种数据结构变化较大的处理又应该多个UI的情况，处理过程可以写在数据对象的钩子中对其统一处理。
